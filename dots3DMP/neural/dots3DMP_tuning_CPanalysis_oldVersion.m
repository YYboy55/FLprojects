...
%                     I = mod==mods(m) & coh==cohs(o);
                ...
                % fit a line to middle few headings
                I = abs(hdg)<20;
                [p,S] = polyfit(hdg(I),spRate_tun(I),1);
                % calculate standard error from S.R, then use that standard
                % error in a t-test for the hypothesis that slope=0
